...
// Initialize logging before anything else (logs to /tmp/web-search-mcp-logs/server.log)
import { initializeLogger, closeLogger } from './logger.js';
initializeLogger();

// Handle EPIPE errors from MCP SDK
process.on(
  'uncaughtException',
  (error: Error & { code?: string; errno?: number }) => {
    if (error.code === 'EPIPE' || error.errno === -32) {
      // Silently ignore EPIPE errors - just suppress, don't exit
      return;
    }
...
class WebSearchMCPServer {
    ...
    const fullWebSearchHandler = async (args: unknown) => {
      ...
      try {
        ...
        const isLikelyRobustModel =
          ...
          'includeContent' in args &&
          typeof (args as Record<string, unknown>).includeContent === 'boolean';
   ...
   * Used for generating status reports and debugging failed searches.
  ...
  /**
   * Categorizes a single error message into a human-readable category.
   * @param errorMessage - The error message to categorize
   * @returns A string category for the error (e.g., 'Timeout', 'Access denied')
   */
  ...
  private setupGracefulShutdown(): void {
    ...
    // Note: uncaughtException handler is already registered at the top of the file
    // to catch EPIPE errors during MCP initialization
