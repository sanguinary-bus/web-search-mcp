...
export class EnhancedContentExtractor {
  ...
  constructor() {
    this.defaultTimeout = parseInt(
      process.env.DEFAULT_TIMEOUT || String(TIMEOUTS.CONTENT_EXTRACTION),
      10
    );
    ...
    if (isNaN(this.maxContentLength) || this.maxContentLength < 0) {
      ...
      this.maxContentLength = CONTENT_LIMITS.MAX_DEFAULT;
    }

    this.browserPool = getBrowserPool();
    this.fallbackThreshold = parseInt(
      process.env.BROWSER_FALLBACK_THRESHOLD ||
        String(BROWSER.FALLBACK_THRESHOLD),
      10
    );
  ...
  private async extractWithBrowser(
    ...
    try {
      ...
      const baseContextOptions = {
        ...
        deviceScaleFactor:
          Math.random() > RANDOM_BEHAVIOR.DEVICE_SCALE_PROBABILITY ? 1 : 2,
        hasTouch: Math.random() > RANDOM_BEHAVIOR.HAS_TOUCH_PROBABILITY,
      };
      ...
      const contextOptions = isFirefox
        ...
        : {
            ...baseContextOptions,
            isMobile: Math.random() > RANDOM_BEHAVIOR.IS_MOBILE_PROBABILITY,
          };
      ...
      try {
        await page.goto(url, {
          ...
          timeout: Math.min(timeout, TIMEOUTS.BROWSER_MAX),
        });
      } catch (gotoError) {
        ...
        if (
          ...
          await http1Page.goto(url, {
            ...
            timeout: Math.min(timeout, TIMEOUTS.CONTENT_EXTRACTION),
          });
      ...
      await page.mouse.move(
        Math.random() * RANDOM_BEHAVIOR.MOUSE_MOVE_MAX_X,
        Math.random() * RANDOM_BEHAVIOR.MOUSE_MOVE_MAX_Y
      );

      // Reduced wait time for dynamic content
      await page.waitForTimeout(
        RANDOM_BEHAVIOR.DELAY_MIN + Math.random() * RANDOM_BEHAVIOR.DELAY_MAX
      );
  ...
  /**
   * Simulates human-like behavior on a page to avoid bot detection.
   * Performs random mouse movements, scrolls, and occasional clicks.
   * Kept for potential future use in more sophisticated bot detection avoidance.
   * @param page - Playwright page to simulate behavior on
   */
  private async simulateHumanBehavior(page: Page): Promise<void> {
    try {
      // Random mouse movements
      await page.mouse.move(
        Math.random() * RANDOM_BEHAVIOR.MOUSE_MOVE_MAX_X * 8,
        Math.random() * RANDOM_BEHAVIOR.MOUSE_MOVE_MAX_Y * 6
      );

      // Random scroll (common human behavior)
      const scrollY = Math.random() * RANDOM_BEHAVIOR.SCROLL_MAX_Y;
      ...
      if (Math.random() > RANDOM_BEHAVIOR.CLICK_PROBABILITY) {
        try {
          await page.click('body', { timeout: RANDOM_BEHAVIOR.CLICK_TIMEOUT });
        } catch {
  ...
  private isLowQualityContent(content: string): boolean {
    const lowQualityIndicators = [
      content.length < CONTENT_LIMITS.MIN_CONTENT_LENGTH,
  ...
  async extractContentForResults(
    ...
    const resultsToProcess = nonPdfResults.slice(
      0,
      Math.min(targetCount * SEARCH.EXTRA_RESULTS_FACTOR, SEARCH.MAX_RESULTS)
  ...
  private parseContent(html: string): string {
    ...
    for (const selector of contentSelectors) {
      ...
      if ($content.length > 0) {
        ...
        if (mainContent.length > CONTENT_LIMITS.MIN_CONTENT_LENGTH) {
    ...
    if (
      !mainContent ||
      mainContent.length < CONTENT_LIMITS.MIN_CONTENT_LENGTH
    ) {
      console.log(
